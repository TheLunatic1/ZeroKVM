  ...
  async startEdgeDetection() {
    ...
    uIOhook.on('mousemove', async (e) => {
      ...
      if (!this.activeRemoteTarget) {
        ...
        // Lock the mouse exactly at the point where we crossed over
    ...
    uIOhook.on('wheel', (e) => {
      if (this.controlledBy) return;
      if (this.activeRemoteTarget) {
        this.webrtcManager.sendInputMessage(this.activeRemoteTarget, JSON.stringify({ type: 'wheel', amount: e.amount, rotation: e.rotation }));
      }
    });
